...
<!-- End Form Cari -->
			<?php  
				if(isset($_GET['pesan'])){
					if($_GET['pesan'] == "gagal"){
						echo "<font color='red'>DATA GAGAL DIUPDATE</font>";
					}else if($_GET['pesan'] == "berhasil"){
						echo "<font color='green'>Data Berhasil Diupdate</font>";
					}else if($_GET['pesan'] == "addkelasoke"){
						echo "<font color='green'>Data Kelas Berhasil di Simpan</font>";
					}else if($_GET['pesan'] == "kelaslangsung"){
						echo "<font color='green'>Data Telah Tersimpan</font>";
					...
					<tbody>
						<?php 
							...
							if(isset($_POST['pencarian'])){
								...
								$sql = "SELECT * FROM pendaftar where kd_jurusan='TI'";
							}
                        ...
                        <input type="hidden" value="<?php echo $d['id']; ?>" name="id">
							...
							<td>
								<?php echo "
								<a style='width:30px; height: 30px; float:left; margin:0;' class='btn btn-block btn-outline-danger btn-sm' href='module/DaftarUlang/proses_hapus.php?id=$d[id]' onClick='return confirm(\" Yakin ingin menghapus ? \")'><i class='ion ion-android-delete'></i></a>
								"; ?>
							</td>
						</tr>
						<?php
